...
$url		= $data[ 'url' ];
?>
<div id="popup-grid-approve" class="cmt-popup popup popup-admin popup-medium <?= $modal ? 'popup-modal' : null ?>">
	...
	<div class="popup-data <?= isset( $widget->size ) ? "popup-data-$widget->size" : null ?>">
		...
		<div class="popup-content">
			<form cmt-app="<?= $app ?>" cmt-controller="<?= $controller ?>" cmt-action="<?= $action ?>" action="<?= $url ?>">
				...
				<h5 class="align align-center">Are you sure you want to approve the selected <?= $model ?> ?</h5>
				<div class="clear filler-height"></div>
				<div class="align align-center"><input class="frm-element-medium" type="submit" value="Approve" /></div>
			</form>
		</div>
	</div>
</div>
